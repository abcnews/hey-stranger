...
class AspectRatioRegulator extends Component {
  ...
  update() {
    clearTimeout(this.inferOrientationTimeout);
    this.inferOrientationTimeout = setTimeout(() => {
      this.setState(this.inferOrientation());
    }, 0);
    this.setState({
      orientation: null,
      ...this.inferValidity()
  ...
  render() {
    ...
    const { isMaxInvalid, isMinInvalid, orientation } = this.state;
    ...
    return (
      <div
        className={cn(styles.root, {
          ...
          [styles.isCounterClockwise]: orientation === -90
        })}
      >
